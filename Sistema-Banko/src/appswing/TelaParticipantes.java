...
public class TelaParticipantes {
			...
			public void actionPerformed(ActionEvent e) {
				try{
					if (table.getSelectedRow() >= 0){
						...
						Conta p = Fachada.localizarParticipante(nome);
						Correntista ev = Fachada.localizarEvento(Integer.parseInt(id));
	...
	public void listagem() {
		try{
			List<Conta> lista = Fachada.listarParticipantes(textField.getText());
			...
			for(Conta p : lista) {
				...
				if(p.getEventos().isEmpty())
					...
					texto=" ";
					for(Correntista ev : p.getEventos()) 
						texto += ev.getId()+ " " ;
				}
