		...
		try {
			...
			while (arquivo2.hasNextLine()) {
				...
				if (tipo.equals("CONTA")) {
					conta = new Conta(Integer.parseInt(id), Double.parseDouble(saldo), data);
					this.adicionar(conta);
				} 
				else {
					limite = partes[4];
					conta = new ContaEspecial(Integer.parseInt(id),Double.parseDouble(saldo),data, Double.parseDouble(limite));
					this.adicionar(conta);
				}
			}
			arquivo2.close();
